...
if ($checkStmt === false) {
    ...
    if ($lastAlertRow) {
        ...
        if ($lastAlertTime instanceof DateTime) {
            ...
            $lastTimestamp = $lastAlertTime->getTimestamp();
            $nowTimestamp = $now->getTimestamp();
            $secondsSinceLastAlert = $nowTimestamp - $lastTimestamp;
            
            // For testing: 5 minutes = 300 seconds
            // For production: 7 days = 604800 seconds
            $intervalSeconds = 1209600; // CHANGE THIS: 300 = 5 min, 600 = 10 min, 3600 = 1 hour, 604800 = 7 days
            
            error_log("Last passport alert sent: " . $lastAlertTime->format('Y-m-d H:i:s'));
            error_log("Current time: " . $now->format('Y-m-d H:i:s'));
            error_log("Seconds since last alert: $secondsSinceLastAlert");
            error_log("Required interval: $intervalSeconds seconds");
            
            // Send if enough time has passed
            if ($secondsSinceLastAlert >= $intervalSeconds) {
                $sendAlert = true;
                error_log("✓ Will send passport alert - enough time has passed");
            } else {
                $remainingSeconds = $intervalSeconds - $secondsSinceLastAlert;
                error_log("✗ Skipping passport alert - need to wait $remainingSeconds more seconds");
            }
        }
    } else {
        $sendAlert = true;
