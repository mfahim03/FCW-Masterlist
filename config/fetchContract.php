...
$count_extend_sql = "SELECT COUNT(*) as total FROM [Updated_FCW_List].[dbo].[Employee] $baseWhereForCounts AND [Contract] = 'EXTEND'";
$count_extend_stmt = sqlsrv_query($conn2, $count_extend_sql);
$count_extend_row = sqlsrv_fetch_array($count_extend_stmt, SQLSRV_FETCH_ASSOC);
$total_extend_records = $count_extend_row['total'];

$count_not_extend_sql = "SELECT COUNT(*) as total FROM [Updated_FCW_List].[dbo].[Employee] $baseWhereForCounts AND [Contract] = 'NOT EXTEND'";
$count_not_extend_stmt = sqlsrv_query($conn2, $count_not_extend_sql);
